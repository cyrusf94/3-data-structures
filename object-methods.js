...
toDo.addToday("build tommorrows lesson plan");
...
/* 
    ! Challenge
    * utilize a filter method to create a new array with
    presidents who were born after the 1900's
    * utilize your new filtered array & use .forEach to print a 
    string interpolated name of the presidents who remained
*/
...
let nineteenFilter = function(array) {
    let filteredArray = array.filter(obj => obj.year > 1900);
    return filteredArray.forEach(element => {
    console.log(`${element.first} ${element.last}`);
        
    });

}
console.log(nineteenFilter(presidents));
